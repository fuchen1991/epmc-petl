...
import static epmc.petl.sexpression.UtilSExpression.newSExpression;
...
import epmc.petl.sexpression.SExpression;
...
final class InputWriter {
    ...
    private final static String MAXIMIZE = "maximize";
    private final static String MINIMIZE = "minimize";
    private final static String OPT_PRIORITY = "opt.priority";
    ...
    private final static String CHECK_SAT_USING = "check-sat-using qfnra-nlsat";
    ...
    void write(boolean first) {
        try (PrintStream out = new PrintStream(outStream);) {
            ...
            writeObjectives(out);
            //setOption(out, OPT_PRIORITY, BOX);
            if(first)
            {
            	command(out, CHECK_SAT_USING);
            }
            else
            {
    ...
    private void writeVariableDeclarations(PrintStream out)  {
        ...
        for (SMTLibVariable variable : solver.getVariables()) {
            ...
            //OperatorEvaluator isNegInf = ContextValue.get().getEvaluator(OperatorIsNegInf.IS_NEG_INF, TypeReal.get());
    ...
    private void writeObjectives(PrintStream out) {
    	for(int i=0;i<solver.getObjectives().size();i++)
    	{
    		if(solver.getDirections().get(i) == Direction.MAX)
    			command(out, MAXIMIZE, solver.getObjectives().get(i));
    		else if(solver.getDirections().get(i) == Direction.MIN)
    			command(out, MINIMIZE, solver.getObjectives().get(i));
    	}
    }
    ...
    private String computeLogic() {
        ...
        for (SMTLibVariable variable : solver.getVariables()) {
            hasReal = hasReal | TypeReal.is(variable.getType());/////////////////////////////////
