...
public class PropertySolverPOSGCumulateReward implements PropertySolver{
	public final static String IDENTIFIER = "posg-cumulate-reward";
	...
	public boolean canHandle() {
        ...
        if (quantifiedReward.getRewardType() != RewardType.CUMULATIVE) {
            return false;
        }
        return true;
	}
	...
	public StateMap doSolve(Expression property, StateSetExplicit states, boolean min)
	{
		...
		int k = ValueInteger.as(UtilEvaluatorExplicit.evaluate(propertyReward.getTime())).getInt();
        ...
        countMemoryUsage();
        double[] resultValue = UtilCumulateReward.computeProbabilities(min,computeForStates, graph,modelChecker,k,stateReward, transReward);
